...
export function updateLocalGame() {
    ...
    // FIX: Normalise diagonal movement so diagonals = same speed as cardinal
    let dx = 0, dy = 0;
    ...
    let nextY = Math.max(pRadius, Math.min(mapH - pRadius, me.y + moveY));
    if (moveY !== 0 && !checkWallCollision(me.x, nextY, pRadius, allWalls)) me.y = nextY;

    let nextX = Math.max(pRadius, Math.min(mapW - pRadius, me.x + moveX));
