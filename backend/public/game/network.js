...
export function initNetwork() {
    ...
    const onGameUpdate = (d) => {
        ...
        if (state.latestServerData && state.latestServerData.players && d.players && socket) {
            ...
            if (localMe && serverMe) {
                // BUG FIX: Práh byl 150px — to je tak velký, že spolkl všechny serverové
                // opravy (zaháknutí do zdi, smrt, respawn). Sníženo na 10px.
                // Nad 10px server vyhraje a pozice se okamžitě opraví.
                ...
                // Záchrana nábojů (zabrání blikání čítače)
    ...
    // BUG FIX: Odstraněny 'gameState' a 'update' — server nikdy tyto eventy nevysílá.
    // Registrace mrtvých listenerů přidávala zmatek o tom, který event je kanonický.
    ...
    socket.on('gameStateChanged', (data) => {
        ...
        if (data.obstacles && data.obstacles.length > 0) state.localObstacles = data.obstacles;
        if (data.breakables && data.breakables.length > 0) state.localBreakables = data.breakables;
