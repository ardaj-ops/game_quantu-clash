// ============================================================
//  QUANTUM CLASH — render.js
//  PATCH v16 (2026-05-18):
//    ✨ Domain map-wide visual effects (black hole, rotating guns, etc.)
//    ✨ Tab menu mouse unlock button + 3-panel layout
//    ✨ Full stat bars for all player stats
//    ⚡ Grid cached on offscreen canvas, redrawn only on resize
//  PATCH v15: Arena border free bounce, diagonal movement normalised
//  PATCH v14: Tab menu stat bars + card history chips with tooltips
//  PATCH v13: Boss crown, domain visual ring
...
function drawTabMenu(playersData, serverData) {
    const RC = { common:'#9e9e9e',uncommon:'#2ed573',rare:'#2a9df4',epic:'#a335ee',legendary:'#ffaa00',mythic:'#ff4757',exotic:'#eccc68',transcended:'#ff2a7a' };
    const players = Object.values(playersData);
    const alive   = players.filter(p => p.hp > 0).sort((a,b)=>(b.score||0)-(a.score||0));
    const dead    = players.filter(p => p.hp <= 0).sort((a,b)=>(b.score||0)-(a.score||0));
    const ctx     = state.ctx;
    ...
    function panel(x, y, w, h, title, accent) {
        accent = accent || '#45f3ff';
        ctx.fillStyle = 'rgba(9,10,15,0.96)'; ctx.fillRect(x,y,w,h);
        ctx.strokeStyle = accent+'99'; ctx.lineWidth = 1.5;
        ctx.shadowColor = accent; ctx.shadowBlur = 8;
        ctx.strokeRect(x+0.5,y+0.5,w-1,h-1); ctx.shadowBlur = 0;
        ctx.fillStyle = accent; ctx.font = 'bold 11px "Orbitron",sans-serif'; ctx.textAlign='left';
        ctx.fillText(title, x+10, y+18);
        ctx.strokeStyle='rgba(255,255,255,0.07)'; ctx.lineWidth=1;
        ctx.beginPath(); ctx.moveTo(x+6,y+24); ctx.lineTo(x+w-6,y+24); ctx.stroke();
    }

    const rowH = 44;
    // ── PANEL 1: ALIVE PLAYERS ───────────────────────────────────────────────
    const P1W = 260, P1H = Math.max(80, 30+alive.length*rowH+10);
    const P1X = M, P1Y = Math.max(M,(SH-P1H)/2);
    panel(P1X,P1Y,P1W,P1H, `ŽIVÍ  (${alive.length})`, '#2ed573');
    alive.forEach((p,i) => {
        const ry = P1Y+30+i*rowH;
        if(i%2===0){ctx.fillStyle='rgba(255,255,255,0.02)';ctx.fillRect(P1X+2,ry,P1W-4,rowH-2);}
        ctx.beginPath();ctx.arc(P1X+18,ry+16,9,0,TWO_PI);
        ctx.fillStyle=p.color||'#fff';ctx.shadowColor=p.isBoss?'#f1c40f':p.color;ctx.shadowBlur=p.isBoss?14:0;ctx.fill();ctx.shadowBlur=0;
        if(p.isBoss){ctx.font='9px serif';ctx.textAlign='center';ctx.fillStyle='#f1c40f';ctx.fillText('👑',P1X+18,ry+8);}
        ctx.fillStyle='#e8eaf0';ctx.font=(p.isBoss?'bold ':'')+'12px "Inter",sans-serif';ctx.textAlign='left';
        ctx.fillText((p.name||'?')+(p.id===socket?.id?' (TY)':''),P1X+33,ry+14);
        ctx.fillStyle='#f1c40f';ctx.font='bold 13px "Orbitron",sans-serif';
        ctx.fillText(p.score||0,P1X+33,ry+30);
        // HP bar
        const hpR=Math.max(0,(p.hp||0)/(p.maxHp||100));
        const hpC=hpR>0.5?'#2ed573':hpR>0.25?'#f1c40f':'#ff4757';
        ctx.fillStyle='#1a1c24';ctx.fillRect(P1X+80,ry+24,P1W-100,5);
        ctx.fillStyle=hpC;ctx.fillRect(P1X+80,ry+24,(P1W-100)*hpR,5);
        ctx.fillStyle='#555';ctx.font='8px "Inter",sans-serif';
        ctx.fillText(`${Math.round(p.hp||0)}/${p.maxHp||100}`,P1X+P1W-16,ry+30);
    });

    // ── PANEL 2: DEAD PLAYERS ─────────────────────────────────────────────────
    const P2W = 240, P2H = Math.max(60, 30+dead.length*34+10);
    const P2X = M, P2Y = P1Y + P1H + M;
    if (dead.length > 0) {
        panel(P2X,P2Y,P2W,P2H, `MRTVÍ  (${dead.length})`, '#ff4757');
        dead.forEach((p,i) => {
            const ry = P2Y+30+i*34;
            ctx.beginPath();ctx.arc(P2X+16,ry+12,7,0,TWO_PI);
            ctx.fillStyle='#333';ctx.fill();
            ctx.fillStyle='#555';ctx.font='11px "Inter",sans-serif';ctx.textAlign='left';
            ctx.fillText('💀 '+(p.name||'?'),P2X+30,ry+16);
            ctx.fillStyle='#444';ctx.font='bold 11px "Orbitron",sans-serif';ctx.textAlign='right';
            ctx.fillText(p.score||0,P2X+P2W-10,ry+16);
        });
    }

    // ── PANEL 3: FULL STAT BARS (my player) ──────────────────────────────────
    const me = socket && playersData[socket.id];
    const P3W = 260, RP_X = SW - M - P3W;
    const ALL_STATS = [
        ...
        {key:'bulletSpeed',label:'Kul.rychlost',max:80,    color:'#a335ee',inv:false,fmt:v=>Math.round(v)},
        {key:'maxAmmo',    label:'Max munice',  max:30,    color:'#f1c40f',inv:false,fmt:v=>Math.round(v)},
        {key:'reloadTime', label:'Přebíjení',   max:3000,  color:'#ff6348',inv:true, fmt:v=>(v/1000).toFixed(1)+'s'},
    ...
    const P3H = 30+ALL_STATS.length*22+10;
    const P3Y = Math.max(M,(SH-P3H)/2);
    panel(RP_X,P3Y,P3W,P3H,'STATY  ·  MŮJ HRÁČ');
    if(me){
        ALL_STATS.forEach((s,si)=>{
            const val=me[s.key]!==undefined?me[s.key]:0;
            const bY=P3Y+28+si*22, bW=100, bX=RP_X+140;
            const r=s.inv?Math.max(0,1-val/s.max):Math.min(1,val/s.max);
            ctx.fillStyle='#888';ctx.font='9px "Inter",sans-serif';ctx.textAlign='right';
            ctx.fillText(s.label,RP_X+135,bY+9);
            ctx.fillStyle='#1a1c24';ctx.fillRect(bX,bY,bW,7);
            ctx.fillStyle=s.color;ctx.shadowColor=s.color;ctx.shadowBlur=3;
            ctx.fillRect(bX,bY,bW*r,7);ctx.shadowBlur=0;
            ctx.fillStyle=s.color;ctx.font='bold 9px "Inter",sans-serif';ctx.textAlign='left';
            ctx.fillText(s.fmt(val),bX+bW+4,bY+8);
        });
    } else {
        ctx.fillStyle='#555';ctx.font='11px "Inter",sans-serif';ctx.textAlign='center';
        ctx.fillText('(Nejsi ve hře)',RP_X+P3W/2,P3Y+80);
    }

    // ── PANEL 4: CARDS PICKED ALL GAME ───────────────────────────────────────
    const CHIP_W=20,CHIP_H=14,CPR=11;
    const MP_W = SW - P1W - P3W - M*4;
    const MP_X = M + P1W + M;
    const maxCards = Math.max(1,...players.map(p=>(p.pickedCards||[]).length));
    const chipRows = Math.max(1,Math.ceil(maxCards/CPR));
    const perPlayer = 22+chipRows*(CHIP_H+3);
    const MP_H = 30+players.length*perPlayer+10;
    const MP_Y = Math.max(M,(SH-MP_H)/2);

    if(MP_W>120){
        panel(MP_X,MP_Y,MP_W,MP_H,'KARTY  (celá hra)','#a335ee');
        _tabCardRegions = [];
        players.forEach((p,i)=>{
            const ry=MP_Y+28+i*perPlayer;
            if(i%2===0){ctx.fillStyle='rgba(255,255,255,0.015)';ctx.fillRect(MP_X+2,ry,MP_W-4,perPlayer-2);}
            ctx.beginPath();ctx.arc(MP_X+12,ry+10,6,0,TWO_PI);
            ctx.fillStyle=p.hp>0?p.color:'#333';ctx.fill();
            ctx.fillStyle=p.hp>0?'#ccc':'#555';ctx.font='10px "Inter",sans-serif';ctx.textAlign='left';
            ctx.fillText((p.hp<=0?'💀 ':'')+p.name+(p.id===socket?.id?' (TY)':''),MP_X+24,ry+14);
            ...
            if(!cards.length){
                ...
                cards.forEach((card,ci)=>{
                    const col=RC[card.rarity?.toLowerCase()]||'#888';
                    const cr=Math.floor(ci/CPR),cc=ci%CPR;
                    const cx2=MP_X+14+cc*(CHIP_W+2),cy2=ry+18+cr*(CHIP_H+3);
                    ctx.fillStyle=col+'28';ctx.fillRect(cx2,cy2,CHIP_W,CHIP_H);
                    ctx.strokeStyle=col;ctx.lineWidth=0.8;ctx.strokeRect(cx2+0.5,cy2+0.5,CHIP_W-1,CHIP_H-1);
    ...
    // ── MOUSE UNLOCK BUTTON ──────────────────────────────────────────────────
    const btnW=190,btnH=30,btnX=SW/2-btnW/2,btnY=SH-M-btnH;
    ctx.fillStyle=_tabMouseUnlocked?'rgba(69,243,255,0.2)':'rgba(255,255,255,0.06)';
    ctx.fillRect(btnX,btnY,btnW,btnH);
    ctx.strokeStyle=_tabMouseUnlocked?'#45f3ff':'#444';ctx.lineWidth=1;
    ctx.strokeRect(btnX+0.5,btnY+0.5,btnW-1,btnH-1);
    ctx.fillStyle=_tabMouseUnlocked?'#45f3ff':'#777';
    ctx.font='9px "Orbitron",sans-serif';ctx.textAlign='center';
    ctx.fillText(_tabMouseUnlocked?'🔓 KURZOR ODEMKNUT  (klikni pro zamknutí)':'🔒 ODEMKNOUT KURZOR PRO TOOLTIPS',SW/2,btnY+btnH/2+3);
    state._tabUnlockBtn={x:btnX,y:btnY,w:btnW,h:btnH};

    // Mode + target score
    ctx.fillStyle='#444';ctx.font='9px "Orbitron",sans-serif';ctx.textAlign='center';
    ctx.fillText(`${serverData?.gameMode||'FFA'}  ·  Cíl: ${serverData?.maxScore||25} kol`,SW/2,btnY-6);
