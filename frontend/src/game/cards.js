...
const availableCards = [
    ...
    { name: "Trojitá hrozba", rarity: "rare", description: "3 kulky, 1 odraz. -30% Dmg, Přebíjení +0.6s.", apply: (p) => { p.multishot = (p.multishot || 1) + 2; p.spread = 0.12; p.bounces = Math.min(CFG.MAX_CAP_BOUNCES, (p.bounces || 0) + 1); p.damage = Math.max(2, p.damage * 0.70); p.reloadTime = (p.reloadTime || 1500) + 600; } },
    ...
    { name: "Krvavá brokovnice", rarity: "epic", description: "5 kulek, 3% lifesteal. Dmg -60%, pálíš pomalu, přebíjení +1s, -5 Ammo.", apply: (p) => { p.multishot = (p.multishot || 1) + 4; p.spread = 0.2; p.lifesteal = Math.min(CFG.MAX_CAP_LIFESTEAL, (p.lifesteal || 0) + 0.03); p.damage = Math.max(2, p.damage * 0.4); p.fireRate += 150; p.reloadTime = (p.reloadTime || 1500) + 1000; p.maxAmmo = Math.max(1, (p.maxAmmo || 10) - 5); p.ammo = Math.min(p.ammo, p.maxAmmo); } },
